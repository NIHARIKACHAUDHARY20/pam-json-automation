import streamlit as st import json import tempfile import os from pydrive.auth import GoogleAuth from pydrive.drive import GoogleDrive

@st.cache_resource def authenticate_drive(): gauth = GoogleAuth() gauth.LocalWebserverAuth() return GoogleDrive(gauth)

drive = authenticate_drive()

st.set_page_config(page_title="PAM Request JSON Generator", layout="wide") st.title("PAM Request JSON Generator") st.markdown("Paste your full Confluence row (including all fields and values). It will parse the row, convert it to JSON, and upload to your Google Drive.")

pasted_text = st.text_area("Paste Confluence Row Below", height=300)

if st.button("Convert & Upload to Google Drive"): try: rows = pasted_text.strip().split("\n") content_rows = [r.strip() for r in rows if r.strip() != ""][-17:]
